...
def decode_csv(raw: bytes) -> str:
    """Handle real-world government exports: UTF-16 or UTF-8 encodings,
    NUL bytes, Windows/Mac line endings — and detect when the 'CSV'
    is actually a webpage (temporary/expired download link)."""
    if raw.startswith(b"\xff\xfe") or raw.startswith(b"\xfe\xff"):
        text = raw.decode("utf-16", errors="replace")
    else:
        text = raw.decode("utf-8-sig", errors="replace")
        if "\x00" in text:  # UTF-16 without BOM, misread as UTF-8
            text = raw.decode("utf-16", errors="replace")
    text = text.replace("\x00", "")
    head = text.lstrip()[:500].lower()
    if head.startswith("<!doctype") or head.startswith("<html") or "<html" in head:
        sys.exit(
            "[extract] The data address returned a WEBPAGE, not a CSV file.\n"
            "The captured download link is temporary or protected.\n"
            "FIX: switch to Plan B in the deployment guide — download the CSV\n"
            "manually from 'Склучени договори', upload it to the data/ folder\n"
            "of your repository, and set ESPP_CSV_URL to the raw.githubusercontent\n"
...
def sniff_delimiter(text: str) -> str:
    """European exports often use ';' instead of ','. Pick whichever
    separator actually appears most in the header line."""
...
def fetch_contracts():
    ...
    print(f"[extract] delimiter: '{delim}' · columns found: {reader.fieldnames}")
    cols = normalize_headers(reader.fieldnames)
    if not cols:
        sys.exit(
            "[extract] none of the CSV columns matched COLUMN_MAP.\n"
            "Copy the 'columns found' line above and adjust COLUMN_MAP "
            "in pipeline.py so the words match your real headers."
        )
    ...
    while True:
        try:
            ...
            bad += 1
            continue
        g = lambda k: (r.get(cols.get(k, "")) or "").strip()
...
def extract_json(txt: str) -> str:
    """Strip code fences and anything outside the outermost {...}."""
...
def ai_package(item) -> dict:
    ...
    If the returned JSON is malformed, ask Claude once to repair it."""
    prompt = (
        ...
        "say no risk indicators were recorded. Inside the text values NEVER "
        "use the ASCII double-quote character — use „…“ or «…» for any "
        "quotation. Output must start with { and end with }. No markdown, "
        "no preamble, no trailing text.\n\n"
        + json.dumps(item, ensure_ascii=False, default=str)
    )
    txt = extract_json(claude(prompt))
    try:
        return json.loads(txt)
    except json.JSONDecodeError:
        repaired = extract_json(claude(
            "The following text was meant to be valid JSON but is broken "
            "(unescaped quotes or truncation). Return ONLY the corrected, "
            "complete, valid JSON — same content, nothing else:\n\n" + txt
        ))
